...
- **V1 resolution path: source-dump** (per A1 findings). Rationale confirmed empirically during A1 Probes 1+2 on host 185.255.127.52 (see `docs/experiments/2026-04-22-a1-wda-probes/probes-1-2-4-findings.md`):
  - **Maestro's iOS driver itself uses source-dump** — `ios.IOSDevice.viewHierarchy` fires every ~500ms during flow execution. Percy adding source-dumps is **additive traffic of the same kind** Maestro already generates, not novel. Per-element would introduce a new kind of WDA traffic with unknown interactions with Maestro's internal element cache.
  - **Android parity** — `adb-hierarchy.js` is source-dump-based; symmetric design across platforms.
  - **Concurrent-safety** — build 2 passed with concurrent WDA queries running; weak positive signal consistent with source-dump-safety.
  - Per-element is deferred to V1.1 as a potential optimization IF production telemetry shows source-dump p95 latency > 500ms. Kill-switch provides incident-response optionality if the shipped path regresses.
